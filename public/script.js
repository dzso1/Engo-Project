...
const ROUTE_MAP = {
  "overview": "/overview", "quiz": "/contest", "achievements": "/rewards", "tests": "/tests",
...
const REVERSE_ROUTE_MAP = {
  "/": "overview", "/overview": "overview", "/dashboard": "overview", "/results": "overview", "/contest": "quiz", "/quiz": "quiz", "/rewards": "achievements",
...
const STUDENT_VIEWS = new Set(["overview", "quiz", "achievements", "tests", "vocabulary", "errorHealing", "listening-lab", "speaking-lab"]);
const ROLE_HOME = { student: "overview", teacher: "teacher-home", parent: "parent-home", admin: "data-admin" };
...
function switchView(id, pushHistory = true) {
  ...
  if (id === "overview") { renderStudentDashboard(); renderStudentResults(); }
...
window.addEventListener("popstate", e => {
  ...
  switchView((e.state && e.state.viewId) || REVERSE_ROUTE_MAP[path] || "overview", false);
});
...
function applyRole(role, switchPage = true) {
  ...
  switchView(ROLE_HOME[role] || "overview");
}
roleSelect.disabled = true;
document.getElementById("dashboardBtn").addEventListener("click", () => switchView(ROLE_HOME[currentUser?.role] || "overview"));
...
document.getElementById("logoutBtn").addEventListener("click", async () => {
  ...
  views.forEach(v => v.classList.toggle("active", v.id === "overview"));
...
async function renderStudentDashboard(force = false) {
  ...
  if (p.tests.pending) todos.push({ title: `${p.tests.pending} bài đang chờ giáo viên chấm Writing`, desc: "Điểm sẽ cập nhật khi giáo viên chấm xong.", view: "overview" });
...
document.querySelectorAll(".modal-close").forEach(btn => btn.addEventListener("click", () => { closeModal("resultModal"); switchView("overview"); }));
